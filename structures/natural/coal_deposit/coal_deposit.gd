class_name CoalDeposit extends NaturalStructure
...
func _get_build_list() -> Array[StructureModel]:
	if is_robot_mining:
		return [
			StructureModel.create("Coal Deposit", [], null, preload("res://structures/natural/coal_deposit/coal_deposit.png"), _remove_robot.bind())
		]
	else:
...
static func get_probability_model() -> SpawnProbabilityModel:
	return SpawnProbabilityModel.create(
		func(): return preload("res://structures/natural/coal_deposit/coal_deposit.tscn"),
		GRID_SIZE,
		{
